...
/**
 * [DeviceIdentifier] 的线上形式：裸字符串。
 *
 * 身份类型住在 :core:model 且不带注解（rules.md §6.1），所以序列化器留在协议模块，由 DTO 显式指定。
 */
object DeviceIdentifierSerializer : KSerializer<DeviceIdentifier> {
    /** 声明成字符串原语，生成的 JSON 里该字段才是裸字符串。 */
    override val descriptor: SerialDescriptor =
        PrimitiveSerialDescriptor("DeviceIdentifier", PrimitiveKind.STRING)
    ...
    override fun deserialize(decoder: Decoder): DeviceIdentifier = DeviceIdentifier(decoder.decodeString())
}
